...
class Recongnizer:
    ...
    def FaceRecongnition(self , image , threshold = 0.5):
        ...
        try:
            ...
            for face_data in faces:
                try:
                    ...
                    if recognized_name != "Unknown":
                        self.results.append([recognized_name, min_distance , facial_area , self.embeddings[recognized_name]["id"] , test_embeddings])
                    elif recognized_name == "Unknown":
                        self.results.append([recognized_name, min_distance , facial_area , None , test_embeddings])
        
                    print("result appended")
                
                except Exception as e:
                    print(f"Error processing a face: {str(e)}")
                    self.results.append("Face is Detected but error while processing")
            ...
            return "Error in face detection or recognition or no face in the image"
            ...
            if key == ord('c'):
                ...
                for result_data in faces:             
                    if type(result_data) != str:
                        ...
                        cv2.putText(frame , f"{result_data[0]}", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2)
                    else:
